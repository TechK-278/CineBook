...
interface CinemasDirectoryClientProps {
  initialTheatres: Theatre[];
}

export function CinemasDirectoryClient({ initialTheatres }: CinemasDirectoryClientProps) {
  ...
  const chains = ["all", "PVR", "INOX", "Cinépolis", "Miraj", "Independent"];

  const filteredCinemas = useMemo(() => {
    return initialTheatres.filter((cinema) => {
      // 1. City Match
      const matchesCity =
        !initialCity ||
        initialCity.toLowerCase() === "all" ||
        cinema.city.toLowerCase().includes(initialCity.toLowerCase());
  ...
  }, [initialTheatres, initialCity, search, selectedChain]);
