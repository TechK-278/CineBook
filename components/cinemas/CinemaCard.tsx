    ...
    >
      <div>
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {cinema.formats.slice(0, 3).map((fmt, i) => (
            <span
              key={i}
          ...
          {cinema.amenities.slice(0, 2).map((amenity, i) => (
            <span
              key={i}
      ...
      <div className="mt-5 pt-3.5 border-t border-cinebook-border flex items-center justify-between">
        <span className="text-xs text-zinc-400">
          <strong className="text-white">{cinema.movieCount}</strong> Movies Playing
        </span>
